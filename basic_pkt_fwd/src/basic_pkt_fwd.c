...
static pthread_mutex_t dummy_concent = PTHREAD_MUTEX_INITIALIZER; /* control access to the dummy */
...
#define DUMMY_IPOL					"false"


////////////////////////////
static bool is_Dummy = false;
static bool Dummy_Cnt = 0;
struct lgw_pkt_rx_s Dummy_rxpkt[NB_PKT_MAX];


void SaveUpLink(int UpLinkPktCnt, struct lgw_pkt_rx_s *pf){ //Dummy NSJ
	int i;
	//printf("DUMMY SaveUpLink \n ");
	for (i = 0; i < UpLinkPktCnt; i++){
		Dummy_rxpkt[i].rf_chain = pf[i].rf_chain;
		Dummy_rxpkt[i].datarate = pf[i].datarate;
		Dummy_rxpkt[i].freq_hz = pf[i].freq_hz; 	
		Dummy_rxpkt[i].coderate = pf[i].coderate;
		Dummy_rxpkt[i].size = pf[i].size;
		Dummy_rxpkt[i].count_us = pf[i].count_us;
		for (int j = 0; j < Dummy_rxpkt[i].size; j++){
			Dummy_rxpkt[i].payload[j] = pf[i].payload[j];
		}
		//strcpy(Dummy_rxpkt[i].payload , pf[i].payload);
	}
	Dummy_Cnt = UpLinkPktCnt;
	is_Dummy = true;
}
...
void thread_up(void) {
	...
	*(uint32_t *)(buff_up + 8) = net_mac_l;
	...
	while (!exit_sig && !quit_sig) {
		...
		printf("\nJSON up: %s\n", (char *)(buff_up + 12)); /* DEBUG: display JSON payload ++ NSJ */
		...
		pthread_mutex_lock(&dummy_concent); //NSJ MUTEX
		SaveUpLink(nb_pkt, rxpkt);
		pthread_mutex_unlock(&dummy_concent);//NSJ MUTEX
	}
...
void thread_down(void) {
	...
	////////////////////////////////////////////////NSJ uint8_t
	int d; //Dummy_Cnt Loop
	char dummy_buff_down[1000]; //Full dummy down data
	uint8_t raw_dummy_data[500]; // raw payload buff 
	char b64_dummy_data[500]; // encode base64 payload buff
	int data_size = 0; //raw_dummy_data size
		
	int dummy_rf_chain         = DUMMY_RFCH;
	char dummy_datarate[10]    = DUMMY_DATR;
	double dummy_freq          = DUMMY_FREQ;
	char dummy_coderate[10]    = DUMMY_CODR;
	...
	while (!exit_sig && !quit_sig) {
		...
		//printf("DUMMY DummyDownLink \n ");
		for (d = 0; d < Dummy_Cnt;d++){

			dummy_rf_chain = Dummy_rxpkt[d].rf_chain;
			switch (Dummy_rxpkt[d].datarate) {
			case DR_LORA_SF7:
				strcpy(dummy_datarate, "SF7BW125");
				break;
			case DR_LORA_SF8:
				strcpy(dummy_datarate, "SF8BW125");
				break;
			case DR_LORA_SF9:
				strcpy(dummy_datarate, "SF9BW125");
				break;
			case DR_LORA_SF10:
				strcpy(dummy_datarate, "SF10BW125");
				break;
			case DR_LORA_SF11:
				strcpy(dummy_datarate, "SF11BW125");
				break;
			case DR_LORA_SF12:
				strcpy(dummy_datarate, "SF12BW125");
				break;
			default:
				MSG("ERROR: [up] lora packet with unknown datarate\n");
				strcpy(dummy_datarate, "SF?BW125");
				exit(EXIT_FAILURE);
			}
			dummy_freq = ((double)Dummy_rxpkt[d].freq_hz / 1e6);
			switch (Dummy_rxpkt[d].coderate) {
			case CR_LORA_4_5:
				strcpy(dummy_coderate, "4/5");
				break;
			case CR_LORA_4_6:
				strcpy(dummy_coderate, "4/6");
				break;
			case CR_LORA_4_7:
				strcpy(dummy_coderate, "4/7");
				break;
			case CR_LORA_4_8:
				strcpy(dummy_coderate, "4/8");
				break;
			case 0: /* treat the CR0 case (mostly false sync) */
				strcpy(dummy_coderate, "OFF");
				break;
			default:
				MSG("ERROR: [up] lora packet with unknown coderate\n");
				strcpy(dummy_coderate, "?");
				exit(EXIT_FAILURE);
			}
			//strcpy(raw_dummy_data, Dummy_rxpkt[d].payload);
			data_size = Dummy_rxpkt[d].size;
			for (int k = 0; k < data_size; k++){
				raw_dummy_data[k] = Dummy_rxpkt[d].payload[k];
			}
			if (DUMMY_PING) raw_dummy_data[5]++;
			raw_dummy_data[data_size] = 0;
			
			bin_to_b64(raw_dummy_data, data_size, (char *)b64_dummy_data, 341); /* 255 bytes = 340 chars in b64 + null char */

			if (DUMMY_PING){
				sprintf(dummy_buff_down, "{\"txpk\":{\"imme\":%s,\"tmst\":%u,\"freq\":%.6lf,\"rfch\":%d,\"powe\":%d,\"modu\":\"%s\",\"datr\":\"%s\",\"codr\":\"%s\",\"ipol\":%s,\"size\":%d,\"data\":\"%s\"}}",
					"false", Dummy_rxpkt[d].count_us+PING_DELAY, dummy_freq, dummy_rf_chain, DUMMY_POWE, DUMMY_MODU, dummy_datarate, dummy_coderate, DUMMY_IPOL, data_size, b64_dummy_data);
			}
			else{
				sprintf(dummy_buff_down, "{\"txpk\":{\"imme\":%s,\"freq\":%.6lf,\"rfch\":%d,\"powe\":%d,\"modu\":\"%s\",\"datr\":\"%s\",\"codr\":\"%s\",\"ipol\":%s,\"size\":%d,\"data\":\"%s\"}}",
					DUMMY_IMME, dummy_freq, dummy_rf_chain, DUMMY_POWE, DUMMY_MODU, dummy_datarate, dummy_coderate, DUMMY_IPOL, data_size, b64_dummy_data);			
			}
			
			strcpy((char *)(buff_down + 4), dummy_buff_down);
			msg_len = strlen(dummy_buff_down) + 4;
			...
			printf("To LoRa Device Payload : %s \n", str);//NSJ
			if (i != txpkt.size) {
